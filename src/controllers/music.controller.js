...
const addContributor = catchAsync(async (req, res) => {
  ...
  if (!description || description.trim().length < 10) {
    throw new ApiError(httpStatus.BAD_REQUEST, 'Description must be at least 10 characters');
  }
  ...
  if (existingContributor) {
    throw new ApiError(httpStatus.BAD_REQUEST, 'You have already contributed to this music');
  }
...
const uploadMusic = catchAsync(async (req, res) => {
  ...
  const isContribution = req.body.isContribution === true || req.body.isContribution === 'true';
...
const likeMusic = async (req, res) => {
  ...
  try {
    ...
    // Toggle like/unlike
    const alreadyLiked = song.likes.some(id => id.toString() === currentUserId.toString());
    if (alreadyLiked) {
      // UNLIKE: remove user from song.likes and song from user.likedSongs
      song.likes = song.likes.filter(id => id.toString() !== currentUserId.toString());
...
const collectMusic = async (req, res) => {
  ...
  try {
    ...
    if (currentUserId === item.createdBy.toString()) {
      return res.status(400).json({ message: 'You cannot collected your work' });
    }
    // Toggle collect/uncollect
    const alreadyCollected = user.collections.includes(musicId);
    if (alreadyCollected) {
      user.collections = user.collections.filter(id => id.toString() !== musicId.toString());
      await user.save();
      return res.status(200).json({
        message: `${itemType.charAt(0).toUpperCase() + itemType.slice(1)} uncollected successfully`,
      ...
      await user.save();
      return res.status(200).json({
        message: `${itemType.charAt(0).toUpperCase() + itemType.slice(1)} collected successfully`,
        collections: user.collections,
        type: itemType
      });
    }
...
const followUser = catchAsync(async (req, res) => {
  ...
  if (alreadyFollowing) {
    ...
    return res.status(200).json({ message: 'Unfollowed successfully', following: currentUser.following });
  }
  // Follow
  currentUser.following.push(targetUserId);
  await currentUser.save();
  return res.status(200).json({ message: 'Followed successfullys', following: currentUser.following });
});
...
const getFollowingList = catchAsync(async (req, res) => {
  const users = await User.find({ isEmailVerified: true }).select('id name email profilePicture createdAt');
  ...
  const usersWithSpace = (await Promise.all(users.map(async (user) => {
    ...
    if (topWork && Array.isArray(topWork)) {
      totalLikes = topWork.reduce((sum, work) => sum + (work.likesCount || 0), 0);
    }
    ...
    allUsers.forEach(u => {
      if (u.collections && Array.isArray(u.collections)) {
        totalCollect += u.collections.filter(cid => allWorkIds.includes(cid.toString())).length;
      }
    });
  ...
  try {
    if (type === 'lyrics') {
      ...
      if (!lyrics) {
        ...
        lyrics = await LyricsMusic.collection.findOne({ _id: new ObjectId(musicId) });
        ...
        if (lyrics) {
          ...
          const alreadyLiked = (lyrics.likes || []).some(id => id.toString() === currentUserId.toString());
          if (alreadyLiked) {
      ...
      if (alreadyLiked) {
        lyrics.likes = lyrics.likes.filter(id => id.toString() !== currentUserId.toString());
        await lyrics.save();
        return res.status(200).json({
          message: 'Lyrics unliked successfully',
      ...
      if (!song) {
        ...
        if (!shareMusicAsset) {
          ...
          if (!shareMusicCreation) {
            return res.status(404).json({ message: 'ShareMusicAsset not found' });
          }
          if (currentUserId === shareMusicCreation.createdBy.toString()) {
            return res.status(400).json({ message: 'You cannot like your work' });
          }
          const alreadyLiked = shareMusicCreation.likes.some(id => id.toString() === currentUserId.toString());
          if (alreadyLiked) {
            shareMusicCreation.likes = shareMusicCreation.likes.filter(id => id.toString() !== currentUserId.toString());
          } else {
            shareMusicCreation.likes.push(currentUserId);
          }
          await shareMusicCreation.save();
          return res.status(200).json({
            message: alreadyLiked ? 'ShareMusicCreation unliked successfully' : 'ShareMusicCreation liked successfully',
        ...
        const alreadyLiked = shareMusicAsset.likes.some(id => id.toString() === currentUserId.toString());
        if (alreadyLiked) {
          shareMusicAsset.likes = shareMusicAsset.likes.filter(id => id.toString() !== currentUserId.toString());
        } else {
          shareMusicAsset.likes.push(currentUserId);
        }
        await shareMusicAsset.save();
        return res.status(200).json({
          message: alreadyLiked ? 'ShareMusicAsset unliked successfully' : 'ShareMusicAsset liked successfully',
...
const reportContent = catchAsync(async (req, res) => {
  ...
  const existingReport = await reportService.findReport({ userId, type, reportedId: id });
  if (existingReport) {
    return res.status(httpStatus.BAD_REQUEST).json({ message: 'You have already reported this content.' });
  }
...
const searchAllAssets = catchAsync(async (req, res) => {
  ...
  const userSpaces = await require('../models/userSpace.model').find({ 
  ...
  const enrichedMusic = music.map(item => {
    const enrichedItem = {
      ...item,
      userName: userMap[item.createdBy]?.userName || item.userName || 'Unknown User',
    ...
    const originalCreator = {
      ...
      myRole: Array.isArray(item.myRole) ? item.myRole : [item.myRole || 'creator'],
    ...
    if (item.contributors && Array.isArray(item.contributors) && item.contributors.length > 0) {
      ...
      const otherContributors = item.contributors.map(contributor => ({
        ...
        myRole: Array.isArray(contributor.myRole) ? contributor.myRole : [contributor.myRole || ''],
  ...
  const filterByArtist = (item) => item.userName && item.userName !== 'Unknown User' && item.userName.trim() !== '';
  ...
  console.log("🔍 Search results - Music:", musicResult.length, "Gigs:", gigResult.length, "Jobs:", jobResult.length, "Creations:", creationResult.length, "Assets:", assetResult.length, "Total:", result.total);
  res.status(httpStatus.OK).send(result);
});
