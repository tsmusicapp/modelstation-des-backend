...
const reportUser = async (req, res) => {
  ...
  try {
    if (!userId || !reporterId) {
      return res
        ...
        .json({
          success: false,
          message:
            "userId (yang dilaporkan) dan reporterId (pelapor) wajib diisi.",
        });
    }
    ...
    try {
      ...
      if (existingReports >= 5) {
        return res.status(429).json({
          success: false,
          message: `Daily report limit reached. You can report ${
            5 - existingReports
          } more users today.`,
          limitReached: true,
    ...
    res
      .status(200)
      .send({ success: true, message: "User reported successfully." });
  } catch (error) {
    res
      .status(500)
      .send({
